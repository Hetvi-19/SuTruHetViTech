...
interface HistoryItem {
  id: string;
  type: 'prediction' | 'advice' | 'diet' | 'firstaid' | 'medicine-prediction' | 'report-analysis';
  query: string;
  result: string;
  timestamp: any;
}
...
export const History: React.FC<HistoryProps> = ({ activeProfileId }) => {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'prediction': return <Activity className="w-5 h-5 text-brand-primary" />;
      ...
      case 'advice': return <Pill className="w-5 h-5 text-brand-primary" />;
      case 'diet': return <Utensils className="w-5 h-5 text-brand-accent" />;
      case 'firstaid': return <AlertCircle className="w-5 h-5 text-red-500" />;
                        ...
                        <div className="text-text-main leading-relaxed">
                          {(() => {
                            try {
                              ...
                              if (selectedItem.type === 'prediction') {
                                return (
                              ...
                              if (selectedItem.type === 'advice') {
                                return (
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    {result.map((med: any, i: number) => (
                                      <div key={i} className="p-8 rounded-[32px] bg-slate-50 border border-border-light shadow-inner hover:bg-white hover:shadow-xl transition-all duration-500">
                                        <div className="flex items-center gap-4 mb-4">
                                          <div className="p-3 rounded-xl bg-brand-primary/10">
                                            <Pill className="w-6 h-6 text-brand-primary" />
                                          </div>
                                          <div className="font-display font-bold text-xl text-text-main">{med.name}</div>
                                        </div>
                                        <div className="text-xs text-brand-primary mb-4 font-bold uppercase tracking-widest bg-brand-primary/5 px-4 py-2 rounded-xl inline-block">
                                          {med.dosage}
                                        </div>
                                        <p className="text-sm text-text-muted leading-relaxed font-medium">{med.warnings}</p>
                                      </div>
                                    ))}
                                  </div>
                                );
                              }
                              if (selectedItem.type === 'diet') {
                                return (
